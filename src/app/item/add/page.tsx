        ...
        <form onSubmit={submitForm} className="flex flex-col gap-2 mb-36">
            ...
            <div className="flex flex-col text-green-100 overlflow-y-scroll">
                ...
                {CATEGORIES.map((category, index) => {
                    return (
                        <label key={index} className='text-xl w-full md:w-fit py-[3px] flex flex-row gap-2 bg-ikigai-600 bg-opacity-20 my-1 px-2 rounded-xl'><input id={category} name={category} className="text-green-950" type='checkbox'></input>{category}</label>
                    )
                })}
            </div>
